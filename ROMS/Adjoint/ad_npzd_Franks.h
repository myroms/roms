...
!        file is in units of tracer. Since Bio_bak is in tracer units,
...
!             Bio_bak(i,k,ibio)=t(i,j,k,nnew,ibio)*Hz_inv(i,k)
              Bio_bak(i,k,ibio)=t(i,j,k,nnew,ibio)
            END DO
          END DO
        END DO
!
...
!
!  NOTE: We must multiply basic state t by Hz.
!
            IF (cff1.gt.0.0) THEN
              itrmx=idbio(1)
!              cff=t(i,j,k,nnew,itrmx)
              cff=t(i,j,k,nnew,itrmx)*Hz(i,j,k)
              DO ibio=idbio(2),idbio(NBT)
!                IF (t(i,j,k,nnew,ibio).gt.cff) THEN
                IF (t(i,j,k,nnew,ibio)*Hz(i,j,k).gt.cff) THEN
                  itrmx=ibio
!                  cff=t(i,j,k,nnew,ibio)
                  cff=t(i,j,k,nnew,ibio)*Hz(i,j,k)
                END IF
              END DO
!
!  Update new values.
...
!>   &                       tl_Bio(i,k,iZoop)*cff2
              ad_Bio(i,k,iZoop)=ad_Bio(i,k,iZoop)+cff2*ad_Bio(i,k,iNO3_)
...
!>   &                       Bio(i,k,iPhyt)*tl_cff*ZooEC(ng)
              ad_cff=ad_cff+Bio(i,k,iPhyt)*ZooEC(ng)*ad_Bio(i,k,iNO3_)
...
!>   &                       tl_cff*(ZooGA(ng)-ZooEC(ng))
              ad_Bio(i,k,iPhyt)=ad_Bio(i,k,iPhyt)+                      &
     &                (cff2+cff*(ZooGA(ng)-ZooEC(ng)))*ad_Bio(i,k,iSDet)
              ad_cff=ad_cff+Bio(i,k,iPhyt)*(ZooGA(ng)-ZooEC(ng))*       &
     &                                                 ad_Bio(i,k,iSDet)

!>            tl_Bio(i,k,iZoop)=tl_Bio(i,k,iZoop)+                      &
!>   &                       tl_Bio(i,k,iPhyt)*cff*(1.0_r8-ZooGA(ng))   &
!>   &                      +Bio(i,k,iPhyt)*tl_cff*(1.0_r8-ZooGA(ng))

              ad_Bio(i,k,iPhyt)=ad_Bio(i,k,iPhyt)+                      &
     &                         cff*(1.0_r8-ZooGA(ng))*ad_Bio(i,k,iZoop)
              ad_cff=ad_cff+Bio(i,k,iPhyt)*(1.0_r8-ZooGA(ng))*          &
     &                                                ad_Bio(i,k,iZoop)

!>            tl_Bio(i,k,iPhyt)=tl_Bio(i,k,iPhyt)/(1.0_r8+cff+cff2)-    &
!>   &                          tl_cff*Bio(i,k,iPhyt)/(1.0_r8+cff+cff2)

              ad_cff=ad_cff-ad_Bio(i,k,iPhyt)*Bio(i,k,iPhyt)/           &
     &                                                (1.0_r8+cff+cff2)
              ad_Bio(i,k,iPhyt)=ad_Bio(i,k,iPhyt)/(1.0_r8+cff+cff2)
...
!>   &            (cff3+Bio1(i,k,iPhyt)*Bio1(i,k,iPhyt))
              ...
              adfac=ad_cff*cff1/(cff3+Bio1(i,k,iPhyt)*Bio1(i,k,iPhyt))
              ad_Bio(i,k,iZoop)=ad_Bio(i,k,iZoop)+Bio1(i,k,iPhyt)*adfac
              ad_Bio(i,k,iPhyt)=ad_Bio(i,k,iPhyt)+Bio1(i,k,iZoop)*adfac
              ad_Bio(i,k,iPhyt)=ad_Bio(i,k,iPhyt)-                      &
     &                     2.0_r8*Bio1(i,k,iPhyt)*ad_cff*cff/           & 
     &                       (cff3+Bio1(i,k,iPhyt)*Bio1(i,k,iPhyt))
              ad_cff=0.0_r8
...
!
              DO itrc=1,NBT
                ibio=idbio(itrc)
!>              tl_Bio(i,k,ibio)=                                       &
!>   &            (0.5_r8-SIGN(0.5_r8,eps-Bio_bak(i,k,ibio)))*          &
!>   &                              tl_Bio_bak(i,k,ibio)-               &
!>   &                     tl_cff1*(SIGN(0.5_r8,                        &
!>   &                                    REAL(itrmx-ibio,r8)**2)+      &
!>   &                              SIGN(0.5_r8,                        &
!>   &                                   -REAL(itrmx-ibio,r8)**2))
     ...
     &             (0.5_r8-SIGN(0.5_r8,eps-Bio_bak(i,k,ibio)))*         &
     &                                               ad_Bio(i,k,ibio)
                ad_cff1=ad_cff1-ad_Bio(i,k,ibio)*(SIGN(0.5_r8,          &
     &                                    REAL(itrmx-ibio,r8)**2)+      &
     &                              SIGN(0.5_r8,                        &
     &                                   -REAL(itrmx-ibio,r8)**2))
                ad_Bio(i,k,ibio)=0.0_r8
...
!>   &                        tl_Bio_bak(i,k,iSDet)
     ...
     &             (0.5_r8-SIGN(0.5_r8,Bio_bak(i,k,iNO3_)-eps))*ad_cff1
            ad_Bio_bak(i,k,iPhyt)=ad_Bio_bak(i,k,iPhyt)-                &
     &             (0.5_r8-SIGN(0.5_r8,Bio_bak(i,k,iPhyt)-eps))*ad_cff1
            ad_Bio_bak(i,k,iZoop)=ad_Bio_bak(i,k,iZoop)-                &
     &             (0.5_r8-SIGN(0.5_r8,Bio_bak(i,k,iZoop)-eps))*ad_cff1
            ad_Bio_bak(i,k,iSDet)=ad_Bio_bak(i,k,iSDet)-                &
     &             (0.5_r8-SIGN(0.5_r8,Bio_bak(i,k,iSDet)-eps))*ad_cff1
            ad_cff1=0.0_r8
...
!
        DO itrc=1,NBT
          ...
          DO k=1,N(ng)
            DO i=Istr,Iend
!
!  NOTE: Basic state tracer field must be multiplied by Hz.
!
!>            tl_Bio_bak(i,k,ibio)=tl_t(i,j,k,nnew,ibio)*Hz_inv(i,k)    &
!>   &                  +t(i,j,k,nnew,ibio)*Hz(i,j,k)*tl_Hz_inv(i,k)
              ad_t(i,j,k,nnew,ibio)=ad_t(i,j,k,nnew,ibio)+              &
     &                                ad_Bio_bak(i,k,ibio)*Hz_inv(i,k)
              ad_Hz_inv(i,k)=ad_Hz_inv(i,k)+                            &
     &                t(i,j,k,nnew,ibio)*Hz(i,j,k)*ad_Bio_bak(i,k,ibio)
              ad_Bio_bak(i,k,ibio)=0.0_r8
            END DO
          END DO
        END DO
