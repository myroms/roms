...
!                                                                      !
!  This routine includes additional options for:                       !
!                                                                      !
!    * Nearshore radiation waves stresses                              !
...
!
!***********************************************************************
      SUBROUTINE step2d_tile (ng, Istr, Iend, Jstr, Jend,               &
     &                        LBi, UBi, LBj, UBj, UBk,                  &
     &                        krhs, kstp, knew,                         &
#ifdef SOLVE3D
     &                        nstp, nnew,                               &
...
# endif
...
#ifdef NEARSHORE_MELLOR
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: DUons
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: DVoms
#endif
#ifdef UV_ADV
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: u_bar
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: v_bar
#endif
#ifdef UV_VIS4
      ...
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY,NDM2d-1) ::          &
     &                   DiaU2rhs, DiaV2rhs
#endif
...
# endif
     ...
     &              on_u(i,j)
#ifdef NEARSHORE_MELLOR
          DUons(i,j)=0.5_r8*(Drhs(i,j)+Drhs(i-1,j))*ubar_stokes(i,j)*   &
     &               on_u(i,j)
          DUon(i,j)=DUon(i,j)+DUons(i,j)
#endif
        END DO
      END DO
      DO j=-1+J_RANGE+1
     ...
     &              om_v(i,j)
#ifdef NEARSHORE_MELLOR
          DVoms(i,j)=0.5_r8*(Drhs(i,j)+Drhs(i,j-1))*vbar_stokes(i,j)*   &
     &               om_v(i,j)
          DVom(i,j)=DVom(i,j)+DVoms(i,j)
#endif
        END DO
      END DO
#ifdef DISTRIBUTE
...
# ifdef NEARSHORE_MELLOR
              DU_avg1(i,j)=DU_avg1(i,j)-cff1*DUons(i,j)
# endif
...
# ifdef NEARSHORE_MELLOR
              DV_avg1(i,j)=DV_avg1(i,j)-cff1*DVoms(i,j)
# endif
...
#if defined WET_DRY
!
! If wet/drying, compute masks for cells with depth < Dcrit.
!
      DO j=Jstr-1,Jend+1
        DO i=Istr-1,Iend+1
          wetdry(i,j)=1.0_r8
      ...
      DO j=JstrR,JendR
        DO i=IstrR,IendR
          rmask_wet(i,j)=wetdry(i,j)*rmask(i,j)
        END DO
      END DO
      DO j=Jstr,Jend
        DO i=IstrU,Iend
...
!
     ...
     &                      gzeta2(i,j  )))
        ...
        END IF
      END DO
#if defined UV_COR || defined UV_ADV
# ifdef EW_PERIODIC
#  define IU_RANGE IstrU-2,Iend+2
# else
#  define IU_RANGE MAX(IstrU-2,1),MIN(Iend+2,Lm(ng)+1)
# endif
# ifdef NS_PERIODIC
#  define JU_RANGE Jstr-2,Jend+2
# else
#  define JU_RANGE MAX(Jstr-2,0),MIN(Jend+2,Mm(ng)+1)
# endif
# ifdef EW_PERIODIC
#  define IV_RANGE Istr-2,Iend+2
# else
#  define IV_RANGE MAX(Istr-2,0),MIN(Iend+2,Lm(ng)+1)
# endif
# ifdef NS_PERIODIC
#  define JV_RANGE JstrV-2,Jend+2
# else
#  define JV_RANGE MAX(JstrV-2,1),MIN(Jend+2,Mm(ng)+1)
# endif
# ifdef NEARSHORE_MELLOR
      DO j=JU_RANGE
        DO i=IU_RANGE
          u_bar(i,j)=ubar(i,j,krhs)+ubar_stokes(i,j)
        END DO
      END DO
      DO j=JV_RANGE
        DO i=IV_RANGE
          v_bar(i,j)=vbar(i,j,krhs)+vbar_stokes(i,j)
        END DO
      END DO
# else
      DO j=JU_RANGE
        DO i=IU_RANGE
          u_bar(i,j)=ubar(i,j,krhs)
        END DO
      END DO
      DO j=JV_RANGE
        DO i=IV_RANGE
          v_bar(i,j)=vbar(i,j,krhs)
        END DO
      END DO
# endif
#  undef IU_RANGE
...
!-----------------------------------------------------------------------
!
# if defined UV_UPSTREAM
!
!  First-order, upstream differences advection.
!
      DO j=Jstr,Jend
        DO i=IstrU-1,Iend
          UFx(i,j)=0.5_r8*                                              &
     &             (MAX(u_bar(i,j),0.0_r8)*                             &
     &                 (DUon(i-1,j)+DUon(i  ,j))+                       &
     &              MIN(u_bar(i,j),0.0_r8)*                             &
     &                 (DUon(i  ,j)+DUon(i+1,j)))
        END DO
      END DO
      DO j=Jstr,Jend+1
        DO i=IstrU,Iend
          UFe(i,j)=0.5_r8*                                              &
     &             (MAX(DVom(i-1,j)+DVom(i,j),0.0_r8)*                  &
     &                 u_bar(i,j-1)+                                    &
     &              MIN(DVom(i-1,j)+DVom(i,j),0.0_r8)*                  &
     &                 u_bar(i,j  ))
        END DO
      END DO
      DO j=JstrV,Jend
        DO i=Istr,Iend+1
          VFX(i,j)=0.5_r8*                                              &
     &             (MAX(DUon(i,j-1)+DUon(i,j),0.0_r8)*                  &
     &                 v_bar(i-1,j)+                                    &
     &              MIN(DUon(i,j-1)+DUon(i,j),0.0_r8)*                  &
     &                 v_bar(i  ,j))
        END DO
      END DO
      DO j=JstrV-1,Jend
        DO i=Istr,Iend
          VFe(i,j)=0.5_r8*                                              &
     &             (MAX(v_bar(i,j),0.0_r8)*                             &
     &                 (DVom(i,j)+DVom(i,j-1))+                         &
     &              MIN(v_bar(i,j),0.0_r8)*                             &
     &                 (DVom(i,j)+DVom(i,j+1)))
        END DO
      END DO
...
#  endif
      DO j=Jstr,Jend
        DO i=IU_RANGE
          grad (i,j)=u_bar(i-1,j)-2.0_r8*u_bar(i,j)+                    &
     &               u_bar(i+1,j)
...
!
!  Fourth-order, centered differences u-momentum advection.
!
      cff=1.0_r8/6.0_r8
      DO j=Jstr,Jend
        DO i=IstrU-1,Iend
          UFx(i,j)=0.25_r8*(u_bar(i,j)+u_bar(i+1,j)-                    &
...
!
     ...
     &             (DUon(i,j)+Duon(i+1,j)+                              &
     &              Gadv*0.5_r8*(Dgrad(i,j)+Dgrad(i+1,j)))
        END DO
      END DO
#  endif
#  ifdef NS_PERIODIC
...
#  endif
      DO j=JU_RANGE
        DO i=IstrU,Iend
          grad(i,j)=u_bar(i,j-1)-2.0_r8*u_bar(i,j)+                     &
     &              u_bar(i,j+1)
        END DO
      END DO
#  undef JU_RANGE
...
#  ifdef UV_C4ADVECTION
      ...
      DO j=Jstr,Jend+1
        DO i=IstrU,Iend
          UFe(i,j)=0.25_r8*(u_bar(i,j)+u_bar(i,j-1)-                    &
...
#  else
      DO j=Jstr,Jend+1
        DO i=IstrU,Iend
          cff1=u_bar(i,j)+u_bar(i,j-1)
          cff2=DVom(i,j)+DVom(i-1,j)
     ...
     &             (cff2+Gadv*0.5_r8*(Dgrad(i,j)+Dgrad(i-1,j)))
        END DO
      END DO
#  endif
#  ifdef EW_PERIODIC
...
#  endif
      DO j=JstrV,Jend
        DO i=IV_RANGE
          grad(i,j)=v_bar(i-1,j)-2.0_r8*v_bar(i,j)+                     &
     &              v_bar(i+1,j)
        END DO
      END DO
#  undef IV_RANGE
...
!
      ...
      DO j=JstrV,Jend
        DO i=Istr,Iend+1
          VFx(i,j)=0.25_r8*(v_bar(i,j)+v_bar(i-1,j)-                    &
     &                      cff*(grad (i,j)+grad (i-1,j)))*             &
     &                     (DUon(i,j)+DUon(i,j-1)-                      &
...
!
      DO j=JstrV,Jend
        DO i=Istr,Iend+1
          cff1=v_bar(i,j)+v_bar(i-1,j)
          cff2=DUon(i,j)+DUon(i,j-1)
          IF (cff2.gt.0.0_r8) THEN
            cff=grad(i-1,j)
          ELSE
            cff=grad(i,j)
     ...
     &             (cff2+Gadv*0.5_r8*(Dgrad(i,j)+Dgrad(i,j-1)))
        END DO
      END DO
#  endif
#  ifdef NS_PERIODIC
...
#  endif
      DO j=JV_RANGE
        DO i=Istr,Iend
          grad(i,j)=v_bar(i,j-1)-2.0_r8*v_bar(i,j)+                     &
     &              v_bar(i,j+1)
...
#  ifdef UV_C4ADVECTION
      ...
      DO j=JstrV-1,Jend
        DO i=Istr,Iend
          VFe(i,j)=0.25_r8*(v_bar(i,j)+v_bar(i,j+1)-                    &
     &                      cff*(grad (i,j)+grad (i,j+1)))*             &
     &                     (DVom(i,j)+DVom(i,j+1)-                      &
     &                      cff*(Dgrad(i,j)+Dgrad(i,j+1)))
        END DO
...
#  else
     ...
     &             (DVom(i,j)+DVom(i,j+1)+                              &
     &              Gadv*0.5_r8*(Dgrad(i,j)+Dgrad(i,j+1)))
        END DO
      END DO
#  endif
# endif
...
# endif
# if (defined CURVGRID && defined UV_ADV)
     &       +0.5_r8*((v_bar(i,j)+v_bar(i  ,j+1))*dndx(i,j)-            &
     &                (u_bar(i,j)+u_bar(i+1,j  ))*dmde(i,j))            &
# endif
     &        )
          UFx(i,j)=cff*(v_bar(i,j)+v_bar(i  ,j+1))
          VFe(i,j)=cff*(u_bar(i,j)+u_bar(i+1,j  ))
# if defined DIAGNOSTICS_UV
#  ifdef UV_COR
...
!
     ...
     &                      cff*cff1*rhs_ubar(i,j))*fac
            ...
            IF (umask_wet(i,j).eq.1.0_r8) THEN
              IF (wetdry(i-1,j).eq.1.0_r8) THEN
            ...
            END IF
            rhs_ubar(i,j)=(ubar(i,j,knew)/fac-                          &
     ...
     &                      cff*cff1*rhs_vbar(i,j))*fac
            ...
            IF (vmask_wet(i,j).eq.1.0_r8) THEN
              IF (wetdry(i,j-1).eq.1.0_r8) THEN
            ...
            END IF
            rhs_vbar(i,j)=(vbar(i,j,knew)/fac-                          &
     ...
     &                           cff3*rubar(i,j,ptsk)))*fac
            ...
            rhs_ubar(i,j)=((ubar(i,j,knew)/fac-                         &
     &                      ubar(i,j,kstp)*                             &
     &                      (Dstp(i,j)+Dstp(i-1,j)))/cff-               &
     &                     cff2*rubar(i,j,kstp)+                        &
     &                     cff3*rubar(i,j,ptsk))/cff1
#endif
          END DO
        END DO
        DO j=JstrV,Jend
     ...
     &                           cff3*rvbar(i,j,ptsk)))*fac
            ...
            rhs_vbar(i,j)=((vbar(i,j,knew)/fac-                         &
     &                      vbar(i,j,kstp)*                             &
     &                      (Dstp(i,j)+Dstp(i,j-1)))/cff-               &
     &                     cff2*rvbar(i,j,kstp)+                        &
     &                     cff3*rvbar(i,j,ptsk))/cff1
#endif
          END DO
        END DO
      END IF
