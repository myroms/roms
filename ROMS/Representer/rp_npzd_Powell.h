...
!
      ...
      real(r8), dimension(PRIVATE_1D_SCRATCH_ARRAY) :: tl_PARsur
      ...
      real(r8), dimension(PRIVATE_1D_SCRATCH_ARRAY,N(ng)) :: tl_Light
...
!  due to phytoplankton (self-shading coefficient).
!
                Att=EXP(-0.5_r8*(AttSW(ng)+AttPhy(ng)*Bio1(i,k,iPhyt))* &
     &                  (z_w(i,j,k)-z_w(i,j,k-1)))
...
# ifdef TL_IOMS
     &       Att*(1.0_r8+(0.5_r8*AttSW(ng)+AttPhy(ng)*Bio1(i,k,iPhyt))* &
     &               (z_w(i,j,k)-z_w(i,j,k-1)))
# endif
                PAR1=PAR
                PAR=PAR*Att
...
# ifdef TL_IOMS
     &           (cff2+2.0_r8*cff3*Light(i,k)*Light(i,k))*cff4*cff4*cff4
# endif
              cff=Bio1(i,k,iPhyt)*                                      &
     &            cff1*cff4*Light(i,k)/                                 &
...
# endif
     ...
     &             tl_Bio(i,k,iNO3_)*cff/(K_NO3(ng)+Bio1(i,k,iNO3_))-   &
# ifdef TL_IOMS
     &             K_NO3(ng)*cff/(K_NO3(ng)+Bio1(i,k,iNO3_))
# endif
#endif
!             Bio(i,k,iNO3_)=Bio(i,k,iNO3_)/(1.0_r8+cff)
...
#ifdef TL_IOMS
     &            Bio1(i,k,iZoop)*cff1*(EXP(-Ivlev(ng)*Bio1(i,k,iPhyt))*&
     &             (Ivlev(ng)*Bio1(i,k,iPhyt)+1.0_r8)-1.0_r8)/          &
     &              Bio1(i,k,iPhyt)
#endif
!             Bio(i,k,iPhyt)=Bio(i,k,iPhyt)/(1.0_r8+cff)
