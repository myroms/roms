...
# endif
      CALL rp_step2d_tile (ng, Istr, Iend, Jstr, Jend,                  &
...
!
!***********************************************************************
      SUBROUTINE rp_step2d_tile (ng, Istr, Iend, Jstr, Jend,            &
     &                           LBi, UBi, LBj, UBj, UBk,               &
     &                           krhs, kstp, knew,                      &
# ifdef SOLVE3D
     &                           nstp, nnew,                            &
...
# endif
      ...
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_Dgrad
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_Dnew
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_Drhs
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_Drhs_p
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_Dstp
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_DUon
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_DVom
# ifdef UV_VIS4
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_LapU
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_LapV
# endif
      ...
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_gzeta
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_gzeta2
# if defined VAR_RHO_2D && defined SOLVE3D
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_gzetaSA
# endif
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_rhs_ubar
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_rhs_vbar
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_rhs_zeta
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_zeta_new
      real(r8), dimension(PRIVATE_2D_SCRATCH_ARRAY) :: tl_zwrk
...
# endif
     ...
     &              on_u(i,j)
          tl_DUon(i,j)=0.5_r8*((Drhs(i,j)+Drhs(i-1,j))*                 &
     &                         tl_ubar(i,j,krhs)+                       &
     &                         (tl_Drhs(i,j)+tl_Drhs(i-1,j))*           &
     &                         ubar(i,j,krhs))*on_u(i,j)-               &
# ifdef TL_IOMS
     &                 DUon(i,j)
# endif
        END DO
     ...
     &              om_v(i,j)
          tl_DVom(i,j)=0.5_r8*((Drhs(i,j)+Drhs(i,j-1))*                 &
     &                         tl_vbar(i,j,krhs)+                       &
     &                         (tl_Drhs(i,j)+tl_Drhs(i,j-1))*           &
     &                         vbar(i,j,krhs))*om_v(i,j)-               &
# ifdef TL_IOMS
     &                 DVom(i,j)
# endif
        END DO
...
# undef I_RANGE
# undef J_RANGE
# if defined OPT_PERT2D || defined OPT_PERT3D || defined TL_IOMS
!
...
#  endif
     ...
     &                      ubar, vbar,                                 &
     &                      Drhs, DUon, DVom)
      CALL rp_set_DUV_bc_tile (ng, Istr, Iend, Jstr, Jend,              &
     &                         LBi, UBi, LBj, UBj,                      &
     &                         ILB, IUB, JLB, JUB,                      &
     &                         krhs,                                    &
#  ifdef MASKING
...
!>
        CALL mp_exchange2d (ng, iRPM, 3, Istr, Iend, Jstr, Jend,        &
     &                      LBi, UBi, LBj, UBj,                         &
     &                      NghostPoints, EWperiodic, NSperiodic,       &
...
!
     ...
     &                 (ubar(i,j,krhs)+ubar(i+1,j,krhs))+               &
     &                 (DUon(i,j)+DUon(i+1,j))*                         &
     &                 (tl_ubar(i,j,krhs)+tl_ubar(i+1,j,krhs)))-        &
#   ifdef TL_IOMS
     &                UFx(i,j)
#   endif
        END DO
     ...
     &                 (ubar(i,j,krhs)+ubar(i,j-1,krhs))+               &
     &                 (DVom(i,j)+DVom(i-1,j))*                         &
     &                 (tl_ubar(i,j,krhs)+tl_ubar(i,j-1,krhs)))-        &
#   ifdef TL_IOMS
     &                UFe(i,j)
#   endif
        END DO
     ...
     &                 (vbar(i,j,krhs)+vbar(i-1,j,krhs))+               &
     &                 (DUon(i,j)+DUon(i,j-1))*                         &
     &                 (tl_vbar(i,j,krhs)+tl_vbar(i-1,j,krhs)))-        &
#   ifdef TL_IOMS
     &                VFx(i,j)
#   endif
        END DO
     ...
     &                 (vbar(i,j,krhs)+vbar(i,j+1,krhs))+               &
     &                 (DVom(i,j)+DVom(i,j+1))*                         &
     &                 (tl_vbar(i,j,krhs)+tl_vbar(i,j+1,krhs)))-        &
#   ifdef TL_IOMS
     &                VFe(i,j)
#   endif
        END DO
...
#  endif
#  if (defined CURVGRID && defined UV_ADV)
     &          +0.5_r8*((vbar(i,j,krhs)+vbar(i  ,j+1,krhs))*           &
     &                   dndx(i,j)-                                     &
     &                   (ubar(i,j,krhs)+ubar(i+1,j  ,krhs))*           &
     &                   dmde(i,j))                                     &
#  endif
...
#   ifdef TL_IOMS
     &           0.25_r8*Drhs(i,j)*                                     &
     &           ((vbar(i,j,krhs)+vbar(i  ,j+1,krhs))*dndx(i,j)-        &
     &            (ubar(i,j,krhs)+ubar(i+1,j  ,krhs))*dmde(i,j))
#   endif
#  endif
...
!>   &                 ubar, vbar, zeta)
!>
      CALL rp_u2dbc_tile (ng, Istr, Iend, Jstr, Jend,                   &
     &                    LBi, UBi, LBj, UBj,                           &
     &                    krhs, kstp, knew,                             &
     &                    ubar, vbar, zeta,                             &
...
!>   &                 ubar, vbar, zeta)
!>
      CALL rp_v2dbc_tile (ng, Istr, Iend, Jstr, Jend,                   &
     &                    LBi, UBi, LBj, UBj,                           &
     &                    krhs, kstp, knew,                             &
     &                    ubar, vbar, zeta,                             &
     &                    tl_ubar, tl_vbar, tl_zeta)
...
!
      CALL rp_obc_flux_tile (ng, Istr, Iend, Jstr, Jend,                &
     &                       LBi, UBi, LBj, UBj,                        &
     &                       knew,                                      &
#  ifdef MASKING
     &                       umask, vmask,                              &
