...
! This small main program only carves up MPI_COMM_WORLD and then starts
...
!  Force the first model to run on the first 2 processors
      color =1
      if (myrank .lt. 2) then
        color = 0
      endif
...
!  fill second attribute using Av import function
      ...
      do i=1,avsize
        testarray(i)= cos((float(i)/24.) * 3.14)
      enddo
